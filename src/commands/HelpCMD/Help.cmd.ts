...
export class HelpCmd implements Command {
  ...
  public description =
    "With the help command you get all necessary information about commands and their usage.";
  ...
  private sendHelpList(tokens: Token[], token: Token, msg: Message): void {
    ...
    const entriesToken = tokens.find(
      (key: Token) =>
        key.token.startsWith("entries") || key.token.startsWith("e")
    );

    if (entriesToken)
      entriesTokenValue = Number(entriesToken.token.match(/\d+/g).join(""));
