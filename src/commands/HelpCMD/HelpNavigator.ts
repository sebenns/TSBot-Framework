...
export class HelpNavigator
{
    ...
    constructor(currPage: number, entriesPerPage: number)
    {
        ...
        this.firstPage = 1;
        this.lastPage = Math.ceil(Object.keys(CmdHandler.getCmdList()).length / this.entriesPerPage);
        this.currPage = currPage <= this.firstPage ? this.firstPage : currPage >= this.lastPage ? this.lastPage : currPage;
    }
